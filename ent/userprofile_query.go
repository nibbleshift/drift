...
//		GroupBy(userprofile.FieldAvatar).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.UserProfile.Query().
//		Select(userprofile.FieldAvatar).
//		Scan(ctx, &v)
...
func (upq *UserProfileQuery) loadLinks(ctx context.Context, query *PostQuery, nodes []*UserProfile, init func(*UserProfile), assign func(*UserProfile, *Post)) error {
	...
	for _, n := range neighbors {
		fk := n.user_profile_links
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_profile_links" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
