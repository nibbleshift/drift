...
func (pq *PostQuery) loadMentions(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	for _, n := range neighbors {
		fk := n.post_mentions
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_mentions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_mentions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
